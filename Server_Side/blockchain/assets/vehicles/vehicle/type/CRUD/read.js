...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/type', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	request(options, function(error, response, body)
	{
		...
		if (!error && !body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			result.message = vehicle.type;
			tracing.create('EXIT', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/type', result);
			res.send(result)
		}
		else 
		{
			res.status(400)
			var error = {}
			error.message = 'Unable to read type'
			error.v5cID = v5cID;
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/type', error)
			res.send(error)
		}
	});
}
