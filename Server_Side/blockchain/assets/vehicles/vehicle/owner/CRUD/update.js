...
var update = function(req, res)
{
	...
	tracing.create('ENTER', 'PUT blockchain/assets/cargopacks/cargopack/'+v5cID+'/owner', req.body);
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	tracing.create('INFO', 'PUT blockchain/assets/cargopacks/cargopack/'+v5cID+'/owner', 'Updating owner value');
	request(options, function(error, response, body)
	{
		if (!error && !body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			var url = configFile.config.app_url + '/blockchain/assets/cargopacks/'+v5cID+'/owner';
			...
			tracing.create('INFO', 'PUT blockchain/assets/cargopacks/cargopack/'+v5cID+'/owner', 'Achieving consensus');
			...
			var interval = setInterval(function(){
					...
					request(options, function (error, response, body) {
						...
						if (!error && response.statusCode == 200) {
							if(JSON.parse(body).message == newValue)
							{
								...
								tracing.create('EXIT', 'PUT blockchain/assets/cargopacks/cargopack/'+v5cID+'/owner', result);
				...
				else
				{
					...
					error.v5cID = v5cID;
					tracing.create('ERROR', 'PUT blockchain/assets/cargopacks/cargopack/'+v5cID+'/owner', error)
		...
		{
			res.status(400)
			tracing.create('ERROR', 'PUT blockchain/assets/cargopacks/cargopack/'+v5cID+'/owner', 'Unable to update owner. v5cID: '+ v5cID)
			var error = {}
			error.error = true
