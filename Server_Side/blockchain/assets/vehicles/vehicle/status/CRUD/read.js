...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/stat', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	request(options, function(error, response, body)
	{
		...
		if (!error && !body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			result.message = vehicle.stat;
			tracing.create('EXIT', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/stat', result);
			res.send(result)
		}
		else 
		{
			res.status(400)
			var error = {}
			error.message = 'Unable to read stat'
			error.v5cID = v5cID;
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/stat', error)
			res.send(error)
		}
	});
}
