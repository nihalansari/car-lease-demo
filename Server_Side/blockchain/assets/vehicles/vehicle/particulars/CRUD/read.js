...
{	
	...
	tracing.create('ENTER', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/particulars', {});
	configFile = reload(__dirname+'/../../../../../../configurations/configuration.js');
	...
	request(options, function(error, response, body)
	{
		...
		if (!error && !body.hasOwnProperty("error") && response.statusCode == 200)
		{
			...
			result.message = vehicle.particulars;
			tracing.create('EXIT', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/particulars', result);
			res.send(result)
		}
		else 
		{
			res.status(400)
			var error = {}
			error.message = 'Unable to read particulars'
			error.v5cID = v5cID;
			error.error = true;
			tracing.create('ERROR', 'GET blockchain/assets/cargopacks/cargopack/'+v5cID+'/particulars', error)
			res.send(error)
		}
	});
}
