...
{
	tracing.create('ENTER', 'POST blockchain/assets/cargopacks', req.body);
	createV5cID(req, res)
}
...
function createV5cID(req, res)
{
	...
	tracing.create('INFO', 'POST blockchain/assets/cargopacks', "Generated V5cID: "+v5cID);
...
function checkIfAlreadyExists(req, res, v5cID)
{
	...
	tracing.create('INFO', 'POST blockchain/assets/cargopacks', "Checking V5cID is unique");
	...
	{	
		...
		{
			tracing.create('INFO', 'POST blockchain/assets/cargopacks', "V5cID is unique");
		...
		else if (response && response.statusCode == 200)
		{
			if(counter < 10){
				...
				error.error = true;
				error.v5cID = v5cID;
				tracing.create('ERROR', 'POST blockchain/assets/cargopacks', error);
...
{
	configFile = reload(__dirname+'/../../../../configurations/configuration.js');
	tracing.create('INFO', 'POST blockchain/assets/cargopacks', "Creating cargopack with v5cID: "+v5cID);
	res.write(JSON.stringify({"message":"Creating cargopack with v5cID: "+ v5cID})+'&&');
	...
	request(options, function(error, response, body){
		if (!error && response && response.statusCode == 200) {
			...
			tracing.create('INFO', 'POST blockchain/assets/cargopacks', "Achieving consensus");
		...
		{			
			res.status(400)
			var error = {}
			error.message = 'Unable to create cargopack';
			error.error = true;
			error.v5cID = v5cID;
			tracing.create('ERROR', 'POST blockchain/assets/cargopacks', error);
			res.end(JSON.stringify(error))
...
function confirmCreated(req, res, v5cID)
{
	...
	var interval = setInterval(function(){
		if(counter < 15){				
			request(options, function(error, response, body){				
				if (body && !body.hasOwnProperty("error") && response.statusCode == 200){
					...
					tracing.create('EXIT', 'POST blockchain/assets/cargopacks', result);
		...
		else
		{
			...
			var error = {}
			error.error = true;
			error.message = 'Unable to confirm cargopack create. Request timed out.';
			error.v5cID = v5cID;
			res.end(JSON.stringify(error))
			clearInterval(interval);
			tracing.create('ERROR', 'POST blockchain/assets/cargopacks', error)
